...
namespace Lockstep.Data
{
    ...
    public sealed class DataHelper
    {
        public DataHelper(
            ...
            out bool valid)
        {
            Sorts = sorts;                  //里面存储了字段的所有的排序方法
            this.TargetType = targetType;   //字段是一个列表 列表里面数据的类型
            SourceEditor = sourceEditor;    //编辑DataBase类
            this.DisplayName = displayName; //显示的名称
            SourceDatabase = sourceDatabase;//源数据库
            _dataFieldName = dataFieldName; //字段名称
